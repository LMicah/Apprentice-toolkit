...
def get_equipment_items(choice: int, bd_filters: pd.DataFrame, stock: pd.DataFrame, itens_prices: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    if not choice:
        raise ValueError("Por favor, insira uma frota")
    ...
    try:
        ...
        raise ValueError("Por favor, insira uma frota válida")

    if isinstance(equipment_items, pd.Series):
        equipment_items = equipment_items.to_frame().T
    else:
...
def fetch_plans(equipment: str, plans: list[str], df_matrix: pd.DataFrame) -> pd.DataFrame:
    """
